...
module UserCredentialErrorHandling
  ...
  def handle_validation_error(credential, context)
    ...
    raise DAO::ValidationError.new("Validation failed while #{context}", credential.errors, credential)
  end

  def handle_record_not_found(user_id)
    log_record_not_found(user_id)
    raise DAO::RecordNotFound, "UserCredential for user_id #{user_id} not found"
  end

  def handle_record_not_found_by_criteria(criteria)
    log_record_not_found_by_criteria(criteria)
    raise DAO::RecordNotFound, "UserCredential not found for criteria: #{criteria.inspect}"
  end
end
