...
import {
    ...
    fetchAndReload // Neue Utility-Funktion
} from './adminUtils.js'; // Importiere aus adminUtils
...
let currentActionToConfirm = null; // 'activate' oder 'deactivate'
let currentUserIdForAssignments = null; // Wird von initAdminUserAssignments gesetzt

// --- Status-Änderungs-Logik ---
...
async function handleConfirmStatusChange() {
  ...
  await fetchAndReload(
    ...
    statusConfirmModalInstance // Modal im Fehlerfall schließen (wird durch finally im fetchAndReload gemacht)
  );
}

// --- Add Assignment Logik ---
async function openAddAssignmentModal() {
  ...
  try {
    ...
    setHtml(licenseListContainer, ''); // Liste leeren
    ...
    licenses.forEach(license => {
      ...
      // Verwende license.license_name, da du es in die API gepackt hast
...
// --- Delete Assignment Logik ---
function openDeleteAssignmentConfirmModal(assignmentId) {
  currentAssignmentIdToModify = assignmentId;
  // Optional: Den Namen der Lizenz im Bestätigungsmodal anzeigen, falls gewünscht
  // const card = document.querySelector(`.assignment-card[data-assignment-id="${assignmentId}"]`);
  // const productName = card ? card.querySelector('h2.card-title')?.textContent.trim() : 'this assignment';
  // setText('nameOfAssignmentToDeleteSpan', productName); // Braucht ein Span im Modal
...
// --- HAUPTINITIALISIERUNG für diese Seite ---
export function initAdminUserAssignments(userId) {
  ...
  // Event Listener für das Schließen der Modals, um Zustände zurückzusetzen (optional, falls nötig)
  const addModalEl = document.getElementById('addAssignmentModal');
  if (addModalEl) {
    addModalEl.addEventListener('hidden.bs.modal', () => {
        // Ggf. UI-Elemente im Add-Modal zurücksetzen
